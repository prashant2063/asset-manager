                              ...
                              <div class="form-group">
																...
																<div class="col-xs-12 col-sm-9">
																	<select id="AssignedUser" name="AssignedUser" class="select2" data-placeholder="Click to Choose..."  onchange="getData(this.value, 'ActivationCode')" >
																		<option value="">&nbsp;</option>
																		<<?php
																			$stmt = $db->query("SELECT Id, Name FROM profilemaster WHERE Status = 'Y'");
																			while($row = $stmt->fetch())
																			{
																				?>
																				<option value="<?php echo $row['Id'];?>" <?php if($row_getAssigned['AssignedUser']==$row['Id']){ echo "selected = 'true'";}?>> <?php echo $row['Name'];?></option>
																				<?php
																			}
																		 ?>
																	</select>
