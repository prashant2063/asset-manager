...
class User extends Password{
  ...
  public function send_mail($email,$message,$subject)
  {
    ...
    $mail->Username="picasadhiman@gmail.com";
    $mail->Password="";
    $mail->SetFrom('picasadhiman@gmail.com','Asset Manager');
    $mail->AddReplyTo("picasadhiman@gmail.com","Asset Manager");
  ...
  public function TechnicianExists($TechnicianId){

    try {

      $stmt = $this->_db->prepare('SELECT TechnicianNumber FROM new_technicianr WHERE TechnicianNumber = :TechnicianNumber');
      $stmt->execute(array('TechnicianNumber' => $TechnicianId));
      $row = $stmt->fetch();

      return $row['TechnicianNumber'];
  ...
  public function countExpiredLicences()
  {
    ...
    try {
      ...
      $stmt = $this->_db->prepare("select
       COUNT(`assetlicences`.`DateInstalled`) as Total  from (`assetlicences` `assetlicences`
  inner join `new_licence` `new_licence`
       on (`new_licence`.`SerialNumber` = `assetlicences`.`LicenceID`))
 where ((`new_licence`.`ReUsable` = '2') AND DATEDIFF( '$form',`assetlicences`.`DateInstalled`) >   `new_licence`.`Period`)
");
      $stmt->execute();
      $row = $stmt->fetch();
