<?php
  ...
  if(!$user->LicenceExists($ActivationCode) == $ActivationCode){
    ...
    $stmt = $db->prepare('INSERT INTO new_licence (Name,Vendor, SerialNumber, ReUsable, Slots, SlotsUsed, Period, DateAdded, AddedBy) VALUES (:Name, :Vendor, :SerialNumber, :ReUsable, :Slots, :SlotsUsed, :Period, :DateAdded, :AddedBy)') ;
    $stmt->execute(array(
     ':Name' => $LicenceName,
